...
public:
  ...
  dim4(dim4 &&other) = default;

  /// Default move assignment operator
  ///
  /// \param[in] other The dim4 that will be moved
  dim4 &operator=(dim4 other) noexcept;
...
/// \returns the reference to the \p ostr after the dim4 string as been streamed
/// in
static inline std::ostream &operator<<(std::ostream &ostr, const dim4 &dims) {
  ostr << dims[0] << " " << dims[1] << " " << dims[2] << " " << dims[3];
  return ostr;
}
...
/// \return The \p istr object after 4 dim_t values have been read from the
/// input
static inline std::istream &operator>>(std::istream &istr, dim4 &dims) {
  istr >> dims[0] >> dims[1] >> dims[2] >> dims[3];
  return istr;
}
...
// /// Returns the number of elements that will be represented by seq if applied
// on an array dim_t calcDim(const af_seq& seq, const dim_t& parentDim);
