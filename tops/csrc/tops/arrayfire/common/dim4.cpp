...
dim4::dim4(dim_t first, dim_t second, dim_t third, dim_t fourth) : dims{first, second, third, fourth} {}

dim4::dim4(const dim4 &other) : dims{other.dims[0], other.dims[1], other.dims[2], other.dims[3]} {}
...
dim_t &dim4::operator[](const unsigned dim) { return const_cast<dim_t &>(static_cast<const dim4 &>((*this))[dim]); }
...
// bool hasEnd(const af_seq& seq)
// {
//     return (seq.begin <= -1 || seq.end <= -1);
// }
...
dim4 calcStrides(const dim4 &parentDim) {
  ...
  //   for (int i = 0; i < 4; i++) {
  //     dim_str.append(std::to_string(parent_dims[i])).append(",");
  //     stride_str.append(std::to_string(out_dims[i])).append(",");
  //   }
  //   printf("dims:%s, strides:%s\n", dim_str.c_str(), stride_str.c_str());
  return out;
}
