...
void cuda_cumsum_tensor(CTensorView input, uint32_t dim, cudaStream_t stream,
  ...
  case ScalarType::DATA_F16: {
    ...
    arrayfire::cuda::scan<af_add_t, __half, __half>(
        output_array, input_array, dim, true, getCudaDeviceProp(), stream);
    break;
  }
  case ScalarType::DATA_F32: {
    ...
    arrayfire::cuda::scan<af_add_t, float, float>(
        output_array, input_array, dim, true, getCudaDeviceProp(), stream);
    break;
  }
  case ScalarType::DATA_F64: {
    ...
    arrayfire::cuda::scan<af_add_t, double, double>(
        output_array, input_array, dim, true, getCudaDeviceProp(), stream);
    break;
  }
  default: {
