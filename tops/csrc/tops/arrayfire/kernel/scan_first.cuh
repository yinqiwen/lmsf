...
namespace arrayfire {
namespace cuda {
using arrayfire::cuda::kernel::THREADS_PER_BLOCK;

template <typename Ti, typename To, af_op_t op, uint32_t DIMX>
__global__ void scan_first_kernel(Param<To> out, Param<To> tmp, CParam<Ti> in,
                                  uint32_t blocks_x, uint32_t blocks_y,
                                  uint32_t lim, bool isFinalPass,
                                  bool inclusive_scan) {
...
#pragma unroll
    for (int off = 1; off < DIMX; off *= 2) {
      if (tidx >= off)
        val = binop(val, sptr[(start - off) + tidx]);
      start = DIMX - start;
      sptr[start + tidx] = val;
...
__global__ void scan_first_bcast_kernel(Param<To> out, CParam<To> tmp,
                                        uint32_t blocks_x, uint32_t blocks_y,
                                        uint32_t lim, bool inclusive_scan) {
  const int tidx = threadIdx.x;
  const int tidy = threadIdx.y;
