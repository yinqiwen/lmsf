...
#include "tops/arrayfire/common/math.h"
...
kerTopkDim0(Param<T> ovals, Param<uint> oidxs, CParam<T> ivals,
            CParam<uint32_t> iidxs, const int k,
            const arrayfire::topkFunction order, uint32_t numLaunchBlocksY) {
  ...
  struct keyValBlocks {
    ...
    T keys[TOPK_IDX_THRD_LOAD * TOPK_THRDS_PER_BLK];
  ...
  // const data_t<T>* kdata = ivals.ptr + by * ivals.strides[1] + bz *
  // ivals.strides[2] + bw * ivals.strides[3];
...
#pragma unroll
  for (uint li = 0, i = blockOffset; li < TOPK_IDX_THRD_LOAD;
    ...
    if (i < elements) {
      ...
      //     maxval<compute_t<T>>();
      smem.blkt.keys[li * TOPK_THRDS_PER_BLK + threadIdx.x] =
          (order & AF_TOPK_MAX) ? arrayfire::cuda::minval<T>()
                                : arrayfire::cuda::maxval<T>();
      smem.blkt.vals[li * TOPK_THRDS_PER_BLK + threadIdx.x] =
          arrayfire::cuda::maxval<ValueType>();
    }
  }
  __syncthreads();
...
#pragma unroll
  ...
  if (order & AF_TOPK_MAX) {
    BlockRadixSortT(smem.sortmem).SortDescendingBlockedToStriped(keys, vals);
  } else {
...
template <typename T>
void topkDim0(Param<T> ovals, Param<uint32_t> oidxs, CParam<T> ivals,
              const int k, const arrayfire::topkFunction order,
              cudaStream_t stream) {
  ...
  Array<T> tvals = arrayfire::createEmptyArray<T>(dim4());
  Array<uint32_t> tidxs = arrayfire::createEmptyArray<uint32_t>(dim4());

  if (numBlocksX > 1) {
    tvals = arrayfire::createEmptyArray<T>(dim4(k * numBlocksX, ivals.dims[1]));
    // TODO(umar): this can be smaller because the first iteration is not
    // reading this array.
    tidxs = arrayfire::createEmptyArray<uint32_t>(
        dim4(k * numBlocksX, ivals.dims[1]));
  }

  int prevBlocksX = 1;

  CParam<T> iivals = ivals;
  CParam<uint> iiidxs = tidxs.GetKernelParams();

  int dims0 = tvals.dims()[0];
  bool first_run = true;
  do {
    if (blocks.x == 1) {
      tvals = arrayfire::createParamArray(ovals, false);
      tidxs = arrayfire::createParamArray(oidxs, false);
    }
    ...
    if (first_run) {
      ...
      // memory
      // CUDA_LAUNCH(
      //     (kerTopkDim0<T, false>), blocks, threads, tvals, tidxs, iivals,
      //     iiidxs, k, order, ivals.dims[1]);
      kerTopkDim0<T, false><<<blocks, threads, 0, stream>>>(
          tvals.GetKernelParams(), tidxs.GetKernelParams(), iivals, iiidxs, k,
          order, ivals.dims[1]);
      first_run = false;
    } else {
      // CUDA_LAUNCH((kerTopkDim0<T, true>), blocks, threads, tvals, tidxs,
      // iivals, iiidxs, k, order, ivals.dims[1]);
      kerTopkDim0<T, true><<<blocks, threads, 0, stream>>>(
          tvals.GetKernelParams(), tidxs.GetKernelParams(), iivals, iiidxs, k,
          order, ivals.dims[1]);
    }
    ...
    iivals = tvals.GetKernelParams();
    iiidxs = tidxs.GetKernelParams();
...
inline void topk(Param<T> ovals, Param<uint> oidxs, CParam<T> ivals,
                 ...
                 const arrayfire::topkFunction order, cudaStream_t stream) {
  assert(dim == 0);
  // TODO Add switch statement when support for other dims is added
  topkDim0<T>(ovals, oidxs, ivals, k, order, stream);
}
} // namespace kernel
} // namespace cuda
} // namespace arrayfire
