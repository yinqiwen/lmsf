...
#include "tops/arrayfire/kernel/tile.cuh"
...
template <typename T>
void tile(Param<T> out, CParam<T> in, const cudaDeviceProp *props, cudaStream_t stream) {
  constexpr unsigned TX = 32;
  constexpr unsigned TY = 8;
  constexpr unsigned TILEX = 512;
  constexpr unsigned TILEY = 32;

  //   auto tile = common::getKernel("arrayfire::cuda::tile", {{tile_cuh_src}}, TemplateArgs(TemplateTypename<T>()));
  ...
  arrayfire::cuda::tile_kernel<T><<<blocks, threads, 0, stream>>>(out, in, blocksPerMatX, blocksPerMatY);
  // POST_LAUNCH_CHECK();
}
