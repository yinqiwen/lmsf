...
#include "tops/arrayfire/common/thrust_utils.h"
#include "tops/arrayfire/kernel/thrust_sort_by_key.h"
...
void thrustSortByKey(Tk *keyPtr, Tv *valPtr, int elements, bool isAscending) {
  if (isAscending) {
    ...
    THRUST_SELECT(thrust::stable_sort_by_key, keyPtr, keyPtr + elements, valPtr,
                  thrust::greater<Tk>());
  }
  // POST_LAUNCH_CHECK();
}
...
//     template void thrustSortByKey<Tk, Tv>(Tk * keyPtr, Tv * valPtr, int elements, bool isAscending);
