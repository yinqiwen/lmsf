...
template <typename T>
__global__ void tile_kernel(Param<T> out, CParam<T> in, const int blocksPerMatX, const int blocksPerMatY) {
  ...
  if (xx >= out.dims[0] || yy >= out.dims[1] || oz >= out.dims[2] || ow >= out.dims[3]) return;
