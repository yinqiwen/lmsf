...
#include "tops/arrayfire/common/binary.h"
#include "tops/arrayfire/common/math.h"
#include "tops/arrayfire/common/optypes.h"
#include "tops/arrayfire/common/param.h"
#include "tops/arrayfire/common/transform.h"
#include "tops/arrayfire/kernel/config.h"

namespace arrayfire {
namespace cuda {
using arrayfire::cuda::kernel::THREADS_X;

template <typename Ti, typename To, af_op_t op, uint32_t DIMY>
__global__ void scan_dim_kernel(Param<To> out, Param<To> tmp, CParam<Ti> in,
                                uint32_t blocks_x, uint32_t blocks_y,
                                uint32_t blocks_dim, uint32_t lim, int dim,
...
#pragma unroll
    for (int off = 1; off < DIMY; off *= 2) {
      if (tidy >= off)
        val = binop(val, sptr[(start - off) * THREADS_X]);
      start = DIMY - start;
...
__global__ void scan_dim_bcast_kernel(Param<To> out, CParam<To> tmp,
                                      uint32_t blocks_x, uint32_t blocks_y,
                                      uint32_t blocks_dim, uint32_t lim,
                                      bool inclusive_scan, int dim) {
  const int tidx = threadIdx.x;
  const int tidy = threadIdx.y;
