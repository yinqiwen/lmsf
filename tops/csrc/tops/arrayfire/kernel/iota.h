...
#include "tops/arrayfire/kernel/iota.cuh"
...
template <typename T>
void iota(Param<T> out, const arrayfire::dim4 &sdims,
  ...
  // blocksPerMatY);
  arrayfire::cuda::iota_kernel<T>
      <<<blocks, threads, 0, stream>>>(out, sdims[0], sdims[1], sdims[2],
                                       sdims[3], blocksPerMatX, blocksPerMatY);
  // POST_LAUNCH_CHECK();
}
