...
#include "tops/tensorrt_llm/common/cudaTypeUtils.cuh"
#include "tops/tensorrt_llm/common/cudaUtils.h"
// #include "tops/tensorrt_llm/common/memory_utils.h"
#include "tops/tensorrt_llm/common/cudaBf16Fallbacks.cuh"
...
template <template <typename T> class Activation, typename T, typename BT>
void invokeGenericActivation(T* out, const BT* bias, const T* gated_weights, const BT* gated_bias, const int* ia3_tasks,
                             const T* ia3_weights, const int m, const int n, const int int8_mode,
...
#define INSTANTIATE_GENERIC_ACTIVATION(Activation, T, BT)                                              \
  template void invokeGenericActivation<Activation, T, BT>(                                            \
      T * out, const BT* bias, const T* gated_weights, const BT* gated_bias, const int* ia3_tasks,     \
      const T* ia3_weights, const int m, const int n, const int int8_mode, const float* activation_in, \
      const float* activation_out, const int* padding_offset, const int seq_len, cudaStream_t stream);
...
#ifdef ENABLE_BF16
template void invokeAddBiasTanh(__nv_bfloat16* out, const __nv_bfloat16* bias, const int m, const int n,
                                cudaStream_t stream);
#endif

template <typename T2, int N>
__global__ void addBiasGeluV2(T2* out, const T2* __restrict bias, const int* ia3_tasks, const T2* ia3_weights,
                              const int size, const int* padding_offset, const int seq_len) {
  const bool with_ia3 = ia3_tasks != nullptr;
...
__global__ void addBiasGeluV3(T2* out, const T2* __restrict bias, const int* ia3_tasks, const T2* ia3_weights,
                              const int size, const int* padding_offset, const int seq_len) {
  const bool with_ia3 = ia3_tasks != nullptr;
...
#define ADD_BIAS_GELU(HALF_N, ELEMENT_PER_ROUND)                                                                    \
  case HALF_N:                                                                                                      \
    if (ELEMENT_PER_ROUND > 1) {                                                                                    \
      ...
      addBiasGeluV2<T2, HALF_N><<<grid, block, 0, stream>>>((T2*)out, (const T2*)bias, ia3_tasks, (T2*)ia3_weights, \
                                                            m * half_n, padding_offset, seq_len);                   \
    }                                                                                                               \
    break;

template <typename T>
void invokeAddBiasGeluV2(T* out, const T* bias, const int* ia3_tasks, const T* ia3_weights, const int* padding_offset,
                         const int seq_len, const int m, const int n, cudaStream_t stream) {
  if (n % 2 == 0 && sizeof(T) == 2) {
...
template void invokeAddBiasGeluV2(float* out, const float* bias, const int* ia3_tasks, const float* ia3_weights,
                                  const int* padding_offset, const int seq_len, const int m, const int n,
                                  cudaStream_t stream);
template void invokeAddBiasGeluV2(half* out, const half* bias, const int* ia3_tasks, const half* ia3_weights,
                                  const int* padding_offset, const int seq_len, const int m, const int n,
                                  cudaStream_t stream);
#ifdef ENABLE_BF16
template void invokeAddBiasGeluV2(__nv_bfloat16* out, const __nv_bfloat16* bias, const int* ia3_tasks,
                                  const __nv_bfloat16* ia3_weights, const int* padding_offset, const int seq_len,
