...
#include "tops/tensorrt_llm/common/reduceKernelUtils.cuh"
...
void invokeQuantizeMatrix(T_OUT* output, T_S const* input_scale, T_IN const* input, int64_t numel, int64_t lda,
  ...
  dim3 block(CTA_SIZE);
  if (quantize_mode == QuantizeMode::PER_CHANNEL) {
    scaleMatrix<QuantizeMode::PER_CHANNEL, true><<<grid, block, 0, stream>>>(output, input_scale, input, numel, lda);
  } else if (quantize_mode == QuantizeMode::PER_TOKEN) {
...
void invokeDequantizeMatrix(T_OUT* output, T_S const* input_scale, T_IN const* input, int64_t numel, int64_t lda,
  ...
  dim3 block(CTA_SIZE);
  if (quantize_mode == QuantizeMode::PER_CHANNEL) {
    scaleMatrix<QuantizeMode::PER_CHANNEL, false><<<grid, block, 0, stream>>>(output, input_scale, input, numel, lda);
  } else if (quantize_mode == QuantizeMode::PER_TOKEN) {
    scaleMatrix<QuantizeMode::PER_TOKEN, false><<<grid, block, 0, stream>>>(output, input_scale, input, numel, lda);
  } else if (quantize_mode == QuantizeMode::PER_TENSOR) {
    scaleMatrix<QuantizeMode::PER_TENSOR, false><<<grid, block, 0, stream>>>(output, input_scale, input, numel, lda);
  }
  sync_check_cuda_error();
}
...
template void invokeFakeQuantize<__nv_fp8_e4m3, float, float>(float* dst, const float* src, const int64_t numel,
                                                              cudaStream_t stream);
template void invokeFakeQuantize<float, float, __nv_fp8_e4m3>(float* dst, const __nv_fp8_e4m3* src, const int64_t numel,
                                                              cudaStream_t stream);
template void invokeFakeQuantize<__nv_fp8_e4m3, half, half>(half* dst, const half* src, const int64_t numel,
                                                            cudaStream_t stream);
...
template void invokeFakeQuantize<float, half, float>(half* dst, const float* src, const int64_t numel,
                                                     cudaStream_t stream);
...
#define DEFINE_INVOKE_QUANTIZE_MATRIX(type_out, type_scale, type_in)                                                  \
  template void invokeQuantizeMatrix<type_out, type_scale, type_in>(type_out * output, type_scale const* input_scale, \
                                                                    type_in const* input, int64_t numel, int64_t lda, \
                                                                    QuantizeMode quantize_mode, cudaStream_t stream); \
