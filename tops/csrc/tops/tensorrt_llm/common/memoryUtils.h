...
__inline__ __host__ __device__ std::enable_if_t<std::is_pointer<TDim>::value, T> constexpr flat_index(T const& acc,
                                                                                                      TDim dims,
                                                                                                      T const& index) {
  assert(index < dims[0]);
...
template <unsigned skip = 0, typename T, std::size_t N, typename... TIndices>
__inline__ __host__ __device__ T constexpr flat_index(T const& acc, std::array<T, N> const& dims, T const& index,
                                                      TIndices... indices) {
  static_assert(skip < N);
  static_assert(sizeof...(TIndices) < N - skip, "Number of indices exceeds number of dimensions");
...
template <unsigned skip = 0, typename T, std::size_t N, typename... TIndices>
__inline__ __host__ __device__ T constexpr flat_index(T const& acc, T const (&dims)[N], T const& index,
                                                      TIndices... indices) {
  static_assert(skip < N);
  static_assert(sizeof...(TIndices) < N - skip, "Number of indices exceeds number of dimensions");
...
template <typename T>
__inline__ __host__ __device__ T constexpr flat_index3(T const& index_0, T const& index_1, T const& index_2,
...
__inline__ __host__ __device__ T constexpr flat_index4(T const& index_0, T const& index_1, T const& index_2,
                                                       T const& index_3, T const& dim_1, T const& dim_2,
                                                       T const& dim_3) {
...
template <typename T>
__inline__ __host__ __device__ T constexpr flat_index_strided3(T const& index_0, T const& index_1, T const& index_2,
...
__inline__ __host__ __device__ T constexpr flat_index_strided4(T const& index_0, T const& index_1, T const& index_2,
                                                               T const& index_3, T const& stride_1, T const& stride_2,
                                                               T const& stride_3) {
  assert(index_1 < stride_1 / stride_2);
...
template <typename T_IN, typename T_OUT>
void invokeCudaD2DScaleCpyConvert(T_OUT* tgt, const T_IN* src, const float* scale, bool invert_scale, const size_t size,
