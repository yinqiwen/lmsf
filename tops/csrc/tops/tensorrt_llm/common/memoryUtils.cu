...
#include "tops/tensorrt_llm/common/memoryUtils.h"
...
template <typename T_IN, typename T_OUT>
__global__ void cudaD2DScaleCpyConvert(T_OUT* dst, const T_IN* src, const float* scale, bool invert_scale,
                                       const size_t size) {
...
template bool invokeCheckRange<int>(const int* buffer, const size_t size, int min, int max, bool* d_within_range,
                                    cudaStream_t stream);
