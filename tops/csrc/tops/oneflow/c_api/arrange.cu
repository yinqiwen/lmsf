...
void cuda_arrange_int_tensor(int64_t start, int64_t delta, const int64_t arange_elem_cnt, CTensorView out,
  ...
  switch (out.dtype) {
    case ScalarType::DATA_U8: {
      ...
      func(stream, start, delta, arange_elem_cnt, reinterpret_cast<uint8_t*>(out.ptr));
      break;
    }
    case ScalarType::DATA_U32: {
      oneflow::user_op::ArangeFunctor<uint32_t> func;
      func(stream, start, delta, arange_elem_cnt, reinterpret_cast<uint32_t*>(out.ptr));
      break;
    }
    case ScalarType::DATA_I64: {
      oneflow::user_op::ArangeFunctor<int64_t> func;
      func(stream, start, delta, arange_elem_cnt, reinterpret_cast<int64_t*>(out.ptr));
      break;
    }
...
void cuda_arrange_float_tensor(double start, double delta, const int64_t arange_elem_cnt, CTensorView out,
  ...
  switch (out.dtype) {
    case ScalarType::DATA_F16: {
      ...
      func(stream, static_cast<half>(start), static_cast<half>(delta), arange_elem_cnt,
