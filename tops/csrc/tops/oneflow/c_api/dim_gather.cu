...
void do_cuda_dim_gather(CTensorView in, int dim, CTensorView indices, cudaStream_t stream, CTensorView out) {
  ...
  oneflow::user_op::DimGatherFunctor<IN_T, IDX_T>()(stream, input_nd_helper, index_nd_helper, ndim,
                                                    index_shape.elem_cnt(), dim_length, dim, index, input, output);
}
