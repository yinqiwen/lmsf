...
namespace {
template <typename T>
void do_cuda_masked_fill(CTensorView input, CTensorView mask, T scalar_operand, cudaStream_t stream, CTensorView out) {
  tops::ShapeView out_shape(out);
  oneflow::WhereKernelUtil<T, uint8_t>::WhereXScalar(
      stream, out_shape.elem_cnt(), reinterpret_cast<const uint8_t*>(mask.ptr), scalar_operand,
      reinterpret_cast<const T*>(input.ptr), reinterpret_cast<T*>(out.ptr));
}
}  // namespace
...
void cuda_masked_fill_int(CTensorView input, CTensorView mask, int64_t scalar_operand, cudaStream_t stream,
  ...
  switch (out.dtype) {
    case ScalarType::DATA_U8: {
      do_cuda_masked_fill<uint8_t>(input, mask, static_cast<uint8_t>(scalar_operand), stream, out);
      break;
    }
    case ScalarType::DATA_U32: {
      do_cuda_masked_fill<uint32_t>(input, mask, static_cast<uint32_t>(scalar_operand), stream, out);
      break;
    }
    ...
    default: {
      throw new std::runtime_error("not supported dtype for cuda_masked_fill_int");
    }
  }
}
...
void cuda_masked_fill_float(CTensorView input, CTensorView mask, double scalar_operand, cudaStream_t stream,
  ...
  switch (out.dtype) {
    case ScalarType::DATA_F16: {
      do_cuda_masked_fill<half>(input, mask, static_cast<half>(scalar_operand), stream, out);
      break;
    }
    case ScalarType::DATA_F32: {
      do_cuda_masked_fill<float>(input, mask, static_cast<float>(scalar_operand), stream, out);
      break;
    }
    ...
    default: {
      throw new std::runtime_error("not supported dtype for cuda_masked_fill_float");
    }
  }
}
