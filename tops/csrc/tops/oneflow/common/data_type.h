...
//   static constexpr half_float::half lowest() { return std::numeric_limits<half_float::half>::lowest(); }
...
template <typename T>
struct IsFloat16 : std::integral_constant<bool, (detail::IsFloat16Helper<typename std::remove_cv<T>::type>::value)> {};
...
struct IsCudaComplex
    : std::integral_constant<bool, (detail::IsCudaComplexHelper<typename std::remove_cv<T>::type>::value)> {};
#endif  // WITH_CUDA

// Type Trait: IsFloating

// #define SPECIALIZE_TRUE_FLOATING(type_cpp, type_proto) DEFINE_SPEC(detail::IsFloatingHelper, type_cpp, true)
...
template <typename T>
struct IsFloating : std::integral_constant<bool, (detail::IsFloatingHelper<typename std::remove_cv<T>::type>::value)> {
};

// Type Trait: IsIntegral

// #define SPECIALIZE_TRUE_INTEGRAL(type_cpp, type_proto) DEFINE_SPEC(detail::IsIntegralHelper, type_cpp, true)
// OF_PP_FOR_EACH_TUPLE(SPECIALIZE_TRUE_INTEGRAL, INT_DATA_TYPE_SEQ);
// #undef SPECIALIZE_TRUE_INTEGRAL

// template <typename T>
// struct IsIntegral : std::integral_constant<bool, (detail::IsIntegralHelper<typename std::remove_cv<T>::type>::value)>
// {
// };

// // Type Trait: IsUnsignedIntegral

// #define SPECIALIZE_TRUE_INTEGRAL(type_cpp, type_proto) DEFINE_SPEC(detail::IsUnsignedIntegralHelper, type_cpp, true)
// OF_PP_FOR_EACH_TUPLE(SPECIALIZE_TRUE_INTEGRAL, UNSIGNED_INT_DATA_TYPE_SEQ);
// #undef SPECIALIZE_TRUE_INTEGRAL

template <typename T>
struct IsUnsignedIntegral
    : std::integral_constant<bool, (detail::IsUnsignedIntegralHelper<typename std::remove_cv<T>::type>::value)> {};
...
//                      ALL_DATA_TYPE_SEQ UNSIGNED_INT32_DATA_TYPE_SEQ FLOAT16_DATA_TYPE_SEQ BFLOAT16_DATA_TYPE_SEQ
//                          COMPLEX_DATA_TYPE_SEQ UNSIGNED_INT64_DATA_TYPE_SEQ INT16_DATA_TYPE_SEQ);
