...
#include "tops/oneflow/common/preprocessor_internal.h"
...
#define OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(macro, ...) OF_PP_INTERNAL_SEQ_PRODUCT_FOR_EACH_TUPLE(macro, __VA_ARGS__)
...
#define OF_PP_FOR_EACH_ATOMIC(macro, seq) OF_PP_FOR_EACH_TUPLE(macro, OF_PP_SEQ_MAP(OF_PP_ATOMIC_TO_TUPLE, seq))

#define OF_PP_SEQ_PRODUCT(seq0, ...) OF_PP_INTERNAL_SEQ_PRODUCT(seq0, __VA_ARGS__)

#define OF_PP_SEQ_MAP(macro, seq) OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(OF_PP_I_SEQ_MAP_DO_EACH, (macro), seq)
