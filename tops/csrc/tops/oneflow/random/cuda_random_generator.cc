...
CUDAGenerator::CUDAGenerator(uint64_t seed, int device_index)
  ...
  // CHECK_LT_OR_THROW(device_index, device_count) << "only " << device_count << " cuda devices are visible.";
  ...
  max_block_num_ = device_prop.multiProcessorCount;
  max_thread_num_ = GetThreadNum(device_prop);
}
...
std::tuple<uint64_t, dim3, dim3> CUDAGenerator::CalcExecutionPolicy(int64_t total_elements) {
  ...
  uint32_t blocks_per_sm = device_prop.maxThreadsPerMultiProcessor / block_size;
  grid.x = std::min(static_cast<uint32_t>(device_prop.multiProcessorCount) * blocks_per_sm, grid.x);
  // number of times random will be generated per thread, to offset philox counter in thc random
  // state
  uint64_t counter_offset = ((numel - 1) / (block_size * grid.x * unroll) + 1) * curand4_engine_calls;
...
void CUDAGenerator::GetState(size_t state_size, void* state) const {
  ...
  memcpy(static_cast<uint8_t*>(state) + states_size + seed_size, &philox_offset_per_thread_, offset_size);
}
