...
#include "tops/oneflow/common/device_type.h"
#include "tops/oneflow/kernel/distribution/distribution_template_util.cuh"
...
struct ExponentialTransformFunctor<float, float> {
  ...
  __device__ float operator()(float random_val) const {
    ...
    // args
    float log = static_cast<float>(random_val) >= static_cast<float>(1.) - epsilon / 2 ? -epsilon / 2 : log_rand;
...
struct ExponentialTransformFunctor<double, double> {
  ...
  __device__ double operator()(double random_val) const {
    ...
    // fast __logf approximation can underflow, so set log to -epsilon/2 for 1 or close to 1
    // args
    double log = static_cast<double>(random_val) >= static_cast<double>(1.) - epsilon / 2 ? -epsilon / 2 : log_rand;
...
struct ExponentialTransformFunctor<half, float> {
  ...
  __device__ half operator()(float random_val) const { return static_cast<half>(float_functor(random_val)); }
...
void ExponentialDistribution<DeviceType::kCUDA, double>::operator()(
    cudaStream_t stream, const int64_t elem_cnt, double* dptr,
    const std::shared_ptr<oneflow::ep::CUDAGenerator>& gen) const {
  // CHECK_GT(elem_cnt, 0);
  ...
  auto execution_policy = gen->CalcExecutionPolicy(elem_cnt);
  ...
  ExponentialTransformFunctor<double, double> transform_functor(std::numeric_limits<double>::epsilon(),
                                                                static_cast<double>(lambd_));
  DistributionFunctor<DistributionOp::kUniform2Double> dist_functor;

  DistributionElementwiseGridStrideKernel<double, double, 2, decltype(dist_functor), decltype(transform_functor)>
      <<<grid, block, 0, stream>>>(elem_cnt, seed, offset, dptr, dist_functor, transform_functor);
}

template <>
void ExponentialDistribution<DeviceType::kCUDA, float>::operator()(
    cudaStream_t stream, const int64_t elem_cnt, float* dptr,
    const std::shared_ptr<oneflow::ep::CUDAGenerator>& gen) const {
  // CHECK_GT(elem_cnt, 0);
  // const auto device_index = stream->device()->device_index();
  // auto gen = CHECK_JUST(generator->Get<ep::CUDAGenerator>(device_index));
  ...
  DistributionElementwiseGridStrideKernel<float, float, 4, decltype(dist_functor), decltype(transform_functor)>
      <<<grid, block, 0, stream>>>(elem_cnt, seed, offset, dptr, dist_functor, transform_functor);
}

template <>
void ExponentialDistribution<DeviceType::kCUDA, half>::operator()(
    cudaStream_t stream, const int64_t elem_cnt, half* dptr,
    const std::shared_ptr<oneflow::ep::CUDAGenerator>& gen) const {
  // CHECK_GT(elem_cnt, 0);
  // const auto device_index = stream->device()->device_index();
  // auto gen = CHECK_JUST(generator->Get<ep::CUDAGenerator>(device_index));
  ...
  DistributionElementwiseGridStrideKernel<half, float, 4, decltype(dist_functor), decltype(transform_functor)>
      <<<grid, block, 0, stream>>>(elem_cnt, seed, offset, dptr, dist_functor, transform_functor);
}
