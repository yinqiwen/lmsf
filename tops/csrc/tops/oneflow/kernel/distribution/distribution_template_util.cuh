...
//     INT_DATA_TYPE_SEQ UNSIGNED_INT_DATA_TYPE_SEQ FLOAT16_DATA_TYPE_SEQ HALF_DATA_TYPE_SEQ)
...
template <typename T, typename ComputeType, int unroll_factor, typename Distribution, typename Transform>
// OF_LAUNCH_BOUNDS_2(block_size_bound, grid_size_bound)
__global__ void DistributionElementwiseGridStrideKernel(
    int64_t numel, uint64_t seed, uint64_t offset, T* out_ptr, Distribution dist_func, Transform transform_func)
{
...
#pragma unroll
        for (int ii = 0; ii < unroll_factor; ii++)
        {
            ...
            if (li < numel)
            {
                out_ptr[li] = transform_func(static_cast<ComputeType>((&rand.x)[ii]));
            }
        }
    }
}
