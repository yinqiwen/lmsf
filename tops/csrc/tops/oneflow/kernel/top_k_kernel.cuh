...
__device__ void BitonicSwap(T* data, const int64_t i, const int64_t j, const bool dir, const Compare& comp) {
...
class MinHeap final {
 public:
  __device__ __forceinline__ MinHeap(Entry<T>* data, const int64_t heap_size, const int64_t init_index,
                                     const T init_value)
  ...
  __device__ __forceinline__ void MaxHeapify(int64_t index) {
    while (true) {
      const int64_t left = 2 * index + 1;
      const int64_t right = 2 * index + 2;
      int64_t max = index;
      if (left < heap_size_ && data_[left] > data_[max]) {
        max = left;
      }
      if (right < heap_size_ && data_[right] > data_[max]) {
        max = right;
      }
      if (max == index) {
        return;
...
template <typename T>
__global__ void HeapTopKKernel(const T* in_ptr, const int64_t instance_num, const int64_t instance_size,
                               const int64_t k, const int64_t heap_size, const int64_t init_index, const T init_value,
                               uint32_t* out_ptr) {
  ...
  auto heap = MinHeap<T>(shared_entries + threadIdx.x * heap_size, heap_size, init_index, init_value);
  ...
  BitonicSort(shared_entries, blockDim.x * heap_size, [](const Entry<T>& x, const Entry<T>& y) { return x > y; });
...
template <typename T>
__global__ void HeapMinTopKKernel(const T* in_ptr, const int64_t instance_num, const int64_t instance_size,
                                  const int64_t k, const int64_t heap_size, const int64_t init_index,
                                  const T init_value, uint32_t* out_ptr) {
  extern __shared__ char smem[];
  auto* shared_entries = reinterpret_cast<Entry<T>*>(smem);
...
//                           ctx->stream()->As<ep::CudaStream>()->cuda_stream()>>>(elem_cnt, buf_manager.IndicesPtr(),
//                                                                                 instance_size);
//       SortPairsDescending(in->dptr<T>(), buf_manager.IndicesPtr(), instance_num, instance_size,
//                           buf_manager.TempStoragePtr(), buf_manager.TempStorageBytes(), buf_manager.SortedInPtr(),
//                           buf_manager.SortedIndicesPtr(), ctx->stream()->As<ep::CudaStream>()->cuda_stream());
//       OF_CUDA_CHECK(cudaMemcpy2DAsync(out->mut_dptr<int64_t>(), k * sizeof(int64_t), buf_manager.SortedIndicesPtr(),
//                                       instance_size * sizeof(int64_t), k * sizeof(int64_t), instance_num,
//                                       cudaMemcpyDefault, ctx->stream()->As<ep::CudaStream>()->cuda_stream()));
//     } else {
...
//       int32_t num_heap = PowOf2Floor(kCudaMaxSharedMemoryByteSize / (heap_size * sizeof(Entry<T>)), 16);
...
//           in->dptr<T>(), instance_num, instance_size, k, heap_size, GetMaxVal<int64_t>(), GetMinVal<T>(),
//           out->mut_dptr<int64_t>());
//     }
//   }
