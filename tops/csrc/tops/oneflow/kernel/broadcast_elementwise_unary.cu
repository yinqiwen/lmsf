...
size_t GetPackSize(size_t num_dims, const int64_t* src_dims, const void* src, const int64_t* dst_dims,
                   const void* dst) {
  ...
  for (size_t pack_size = max_pack_size; pack_size > 2; pack_size /= 2) {
    ...
    if (is_src_supported && is_dst_supported) {
      return pack_size;
    }
  }
  return 1;
...
struct UnaryScalarFunctor {
  ...
  __device__ Dst operator()() const { return UnaryFunctor<DeviceType::kCUDA, unary_op, Dst, Src>()(scalar); }
  const Src scalar;
};

template <UnaryOp unary_op, typename Src, typename Dst>
struct UnaryScalarPtrFunctorFactory {
  __host__ __device__ explicit UnaryScalarPtrFunctorFactory(const Src* scalar_ptr) : scalar_ptr(scalar_ptr) {}
...
template <UnaryOp op, typename Src, typename Dst, size_t max_dims, size_t pack_size, typename IndexType>
__global__ void BroadcastElementwiseUnaryGpu(BroadcastElementwiseUnaryParams<Src, Dst, max_dims, IndexType> params) {
...
#pragma unroll
    for (int j = 0; j < pack_size; ++j) {
      dst_pack.elem[j] = functor(src_pack.elem[j]);
    }
...
template <UnaryOp op, typename Src, typename Dst, size_t max_dims, size_t pack_size, typename IndexType>
void LaunchKernel(CudaStream* stream, size_t num_dims, const int64_t* src_dims, const int64_t* src_strides,
                  const Src* src, const int64_t* dst_dims, const int64_t* dst_strides, Dst* dst, bool continuous_output,
                  Scalar attr0, Scalar attr1, size_t count) {
  ...
  params.dst_offset_to_index_helper = OffsetToIndexWithStrideCalculator<IndexType, max_dims>(dst_dims, num_dims);
  ...
  BroadcastElementwiseUnaryGpu<op, Src, Dst, max_dims, pack_size, IndexType>
      <<<BlocksNum4ThreadsNum(params.count), kCudaThreadsNumPerBlock, 0, stream->cuda_stream()>>>(params);
}

template <UnaryOp op, typename Src, typename Dst, size_t max_dims, size_t pack_size>
void DispatchIndexType(CudaStream* stream, size_t num_dims, const int64_t* src_dims, const int64_t* src_strides,
                       const Src* src, const int64_t* dst_dims, const int64_t* dst_strides, Dst* dst,
  ...
  if (count < GetMaxVal<int32_t>() / 2) {
    LaunchKernel<op, Src, Dst, max_dims, pack_size, int32_t>(stream, num_dims, src_dims, src_strides, src, dst_dims,
                                                             dst_strides, dst, continuous_output, attr0, attr1, count);
  } else {
    LaunchKernel<op, Src, Dst, max_dims, pack_size, int64_t>(stream, num_dims, src_dims, src_strides, src, dst_dims,
                                                             dst_strides, dst, continuous_output, attr0, attr1, count);
  }
}

template <UnaryOp op, typename Src, typename Dst, size_t max_dims>
void DispatchPackSize(CudaStream* stream, size_t pack_size, size_t num_dims, const int64_t* src_dims,
                      const int64_t* src_strides, const Src* src, const int64_t* dst_dims, const int64_t* dst_strides,
                      Dst* dst, bool continuous_output, Scalar attr0, Scalar attr1) {
  void (*func)(CudaStream* /*stream*/, size_t /*num_dims*/, const int64_t* /*src_dims*/, const int64_t* /*src_strides*/,
               const Src* /*src*/, const int64_t* /*dst_dims*/, const int64_t* /*dst_strides*/, Dst* /*dst*/,
  ...
  func(stream, num_dims, src_dims, src_strides, src, dst_dims, dst_strides, dst, continuous_output, attr0, attr1);
}
...
void DispatchNumDims(CudaStream* stream, size_t pack_size, size_t num_dims, const int64_t* src_dims,
                     const int64_t* src_strides, const Src* src, const int64_t* dst_dims, const int64_t* dst_strides,
                     Dst* dst, bool continuous_output, Scalar attr0, Scalar attr1) {
  void (*func)(CudaStream* /*stream*/, size_t /*pack_size*/, size_t /*num_dims*/, const int64_t* /*src_dims*/,
  ...
  func(stream, pack_size, num_dims, src_dims, src_strides, src, dst_dims, dst_strides, dst, continuous_output, attr0,
       attr1);
}

template <UnaryOp op, typename Src, typename Dst>
void LaunchWithSimplified(CudaStream* stream, size_t simplified_num_dims, int64_t* simplified_src_dims,
  ...
  if (src_enable_pack && dst_enable_pack) {
    pack_size =
        GetPackSize<kMaxPackSize, Src, Dst>(simplified_num_dims, simplified_src_dims, src, simplified_dst_dims, dst);
  }
  ...
  for (int i = simplified_num_dims - 1; i >= 0; i--) {
    if ((i == simplified_num_dims - 1 && simplified_dst_strides[i] != 1) ||
        (i != simplified_num_dims - 1 &&
         simplified_dst_strides[i] != simplified_dst_strides[i + 1] * simplified_dst_dims[i + 1])) {
  ...
  DispatchNumDims<op, Src, Dst>(stream, pack_size, simplified_num_dims, simplified_src_dims, simplified_src_strides,
                                src, simplified_dst_dims, simplified_dst_strides, dst, continuous_output, attr0, attr1);
}

template <UnaryOp op, typename Src, typename Dst, size_t pack, bool tail>
__global__ void LaunchFillKernel(UnaryFunctor<DeviceType::kCUDA, op, Dst, Src> functor, Dst* dst, const Src* src,
                                 size_t pack_count, size_t count, size_t tail_count, Dst* tail_dst) {
  using StorePack = cuda::elementwise::Packed<Dst, pack>;
  StorePack pack_value;
  Dst value = functor(*src);
#pragma unroll
  for (size_t i = 0; i < pack; ++i) {
    pack_value.elem[i] = value;
  }
...
typename std::enable_if<(pack != 0), void>::type LaunchPackFill(CudaStream* stream, Dst* dst, const Src* src,
...
template <UnaryOp op, typename Src, typename Dst>
void LaunchFill(CudaStream* stream, Dst* dst, const Src* src, size_t count, Scalar attr0, Scalar attr1) {
...
class BroadcastElementwiseUnaryImpl : public BroadcastElementwiseUnary {
 public:
  ...
  void Launch(Stream* stream, size_t num_src_dims, const int64_t* src_dims, const void* src, size_t num_dst_dims,
              const int64_t* dst_dims, void* dst) override {
    ...
    Launch(stream, num_src_dims, src_dims, src_strides, src, num_dst_dims, dst_dims, dst_strides, dst);
  }

  void Launch(Stream* stream, size_t num_src_dims, const int64_t* src_dims, const int64_t* src_strides,
              const void* src_ptr, size_t num_dst_dims, const int64_t* dst_dims, const int64_t* dst_strides,
              void* dst_ptr) override {
    CHECK_GT(num_src_dims, 0) << "num_src_dims must greater than 0";
    CHECK_GT(num_dst_dims, 0) << "num_dst_dims must greater than 0";
    ...
    bool permutable = InferPermutable<kMaxNumDims>(simplified_num_dims, simplified_src_strides, simplified_dst_strides,
                                                   simplified_src_dims, simplified_dst_dims, permutation_list,
                                                   permutation_src_dims, unary_op);
    ...
    } else if (simplified_num_dims == 1 && simplified_src_strides[0] == 1 && simplified_dst_strides[0] == 1) {
      const int64_t elem_cnt = simplified_src_dims[0];
      auto functor = UnaryFunctor<DeviceType::kCUDA, unary_op, Dst, Src>(attr0, attr1);
      OF_CUDA_CHECK((cuda::elementwise::Unary<decltype(functor), Dst, Src>(functor, elem_cnt, dst, src,
                                                                           cuda_stream->cuda_stream())));
    } else if (permutable && src_type == dst_type && permute) {
      permute->Launch(stream, dst_type, simplified_num_dims, permutation_src_dims, src_ptr, permutation_list, dst_ptr);
    } else {
      // fall back to normal cases
      LaunchWithSimplified<unary_op, Src, Dst>(cuda_stream, simplified_num_dims, simplified_src_dims,
                                               simplified_src_strides, src, simplified_dst_dims, simplified_dst_strides,
                                               dst, attr0, attr1);
    }
  }
...
template <UnaryOp unary_op, typename Src, DataType src_type, typename Dst, DataType dst_type>
std::unique_ptr<BroadcastElementwiseUnary> NewBroadcastElementwiseUnary(Scalar attr0, Scalar attr1) {
...
class BroadcastElementwiseUnaryFactoryImpl : public BroadcastElementwiseUnaryFactory {
 public:
  ...
  std::unique_ptr<BroadcastElementwiseUnary> New(UnaryOp op, DataType src_type, DataType dst_type, size_t max_num_dims,
...
#define MAKE_NEW_BROADCAST_ELEMENTWISE_UNARY_ENTRY(unary_op, src_dtype_pair, dst_dtype_pair)                   \
  {std::make_tuple(unary_op, OF_PP_PAIR_SECOND(src_dtype_pair), OF_PP_PAIR_SECOND(dst_dtype_pair)),            \
   NewBroadcastElementwiseUnary<unary_op, OF_PP_PAIR_FIRST(src_dtype_pair), OF_PP_PAIR_SECOND(src_dtype_pair), \
    ...
    static const std::map<std::tuple<UnaryOp, DataType, DataType>,
        ...
        new_broadcast_elementwise_unary_handle{
            // For All Type OP
            OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_NEW_SAME_DTYPE_BROADCAST_ELEMENTWISE_UNARY_ENTRY, UNARY_IDENTITY_SEQ,
                                             CUDA_PRIMITIVE_REAL_TYPE_SEQ)
            ...
            OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_NEW_BROADCAST_ELEMENTWISE_UNARY_ENTRY,
                                             BROADCAST_ELEMENTWISE_CAST_OP_SEQ, CUDA_PRIMITIVE_CAST_REAL_TYPE_SEQ,
                                             CUDA_PRIMITIVE_CAST_REAL_TYPE_SEQ CUDA_PRIMITIVE_COMPLEX_TYPE_SEQ)

                OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_NEW_BROADCAST_ELEMENTWISE_UNARY_ENTRY,
                                                 BROADCAST_ELEMENTWISE_CAST_OP_SEQ, CUDA_PRIMITIVE_COMPLEX_TYPE_SEQ,
...
#undef MAKE_NEW_SAME_DTYPE_BROADCAST_ELEMENTWISE_UNARY_ENTRY
    ...
    const auto iter = new_broadcast_elementwise_unary_handle.find(std::make_tuple(unary_op, src_type, dst_type));
...
REGISTER_PRIMITIVE_FACTORY(DeviceType::kCUDA, BroadcastElementwiseUnaryFactory, BroadcastElementwiseUnaryFactoryImpl);
