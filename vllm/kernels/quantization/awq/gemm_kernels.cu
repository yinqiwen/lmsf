...
void vllm_awq_dequantize(void *kernel_data, void *scaling_factors_data,
                         void *zero_data, void *de_kernel_data,
                         cudaStream_t stream, AWQDequntizeParams params) {
  int in_c = params.in_c;
  int qout_c = params.qout_c;
  int out_c = qout_c * 8;
  int thx = params.thx;
  int thy = params.thy;
  int G = in_c / params.scaling_factors_size;
  ...
  auto de_kernel = reinterpret_cast<half *>(de_kernel_data);
  auto scaling_factors = reinterpret_cast<half *>(scaling_factors_data);
...
void vllm_awq_gemm(void *in_feats_data, void *kernel_data,
  ...
  auto kernel = reinterpret_cast<int *>(kernel_data);
  auto out_feats = reinterpret_cast<half *>(out_feats_data);
  auto scaling_factors = reinterpret_cast<half *>(scaling_factors_data);
  auto zeros = reinterpret_cast<int *>(zero_data);
