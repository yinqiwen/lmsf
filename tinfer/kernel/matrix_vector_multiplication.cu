...
#include "tinfer/kernel/reduce_kernel_utils.cuh"
...
__global__ void int8WeightPerChannelLdkMultiplication(const char4* weight, const float4* input, const float* scale_list,
                                                      void* output, const int k_4) {
...
// grid(n/nPerThread)
template <int m, int nPerThread>
__global__ void int8WeightPerChannelLdkMultiplication(const char4* weight, const half4* input, const float* scale_list,
                                                      void* output, const int k_4) {
  const int tidx = threadIdx.x;
  const int bidx = blockIdx.x;
  const int row_idx = bidx * nPerThread;
...
#ifdef ENABLE_BF16
template <int m, int nPerThread>
__global__ void int8WeightPerChannelLdkMultiplication(const char4* weight, const bf164* input, const float* scale_list,
                                                      void* output, const int k_4) {
  const int tidx = threadIdx.x;
  const int bidx = blockIdx.x;
  const int row_idx = bidx * nPerThread;
...
#define RUN(M, TYPE)                                   \
  int8WeightPerChannelLdkMultiplication<M, nPerThread> \
      <<<grid, block, shm_size, stream>>>((const char4*)weight, (const TYPE*)input, scale_list, (void*)output, k / 4);

template <typename T>
void int8WeightPerChannelLdkMultiplicationLauncher(const int8_t* weight, const T* input, const float* scale_list,
                                                   T* output, const int m, const int n, const int k,
                                                   cudaStream_t stream) {
...
template void int8WeightPerChannelLdkMultiplicationLauncher(const int8_t* matrix, const float* vector,
                                                            const float* scale_list, float* output, const int m,
                                                            const int n, const int k, cudaStream_t stream);

template void int8WeightPerChannelLdkMultiplicationLauncher(const int8_t* matrix, const half* vector,
                                                            const float* scale_list, half* output, const int m,
                                                            const int n, const int k, cudaStream_t stream);

#ifdef ENABLE_BF16
template void int8WeightPerChannelLdkMultiplicationLauncher(const int8_t* matrix, const __nv_bfloat16* vector,
                                                            const float* scale_list, __nv_bfloat16* output, const int m,
                                                            const int n, const int k, cudaStream_t stream);
#endif
/////////////////////////////////////////////////////////////////////
