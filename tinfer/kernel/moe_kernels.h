...
class CutlassMoeFCRunner {
 public:
  ...
  void run_moe_fc(const T* input_activations, const T* gating_output, const WeightType* fc1_expert_weights,
                  const T* fc1_scales, const T* fc1_expert_biases, tinfer::ActivationType fc1_activation_type,
...
class CutlassMoeFCRunner<float, WeightType, typename std::enable_if_t<!std::is_same<float, WeightType>::value>> {
 public:
  ...
  void run_moe_fc(const float* input_activations, const float* gating_output, const uint8_t* fc1_expert_weights,
                  const float* fc1_scales, const float* fc1_expert_biases, tinfer::ActivationType fc1_activation_type,
                  const uint8_t* fc2_expert_weights, const float* fc2_scales, const int num_rows, const int hidden_size,
                  const int inter_size, const int num_experts, const int k, char* workspace_ptr, float* fc2_result,
                  float* expert_scales, int* expanded_source_row_to_expanded_dest_row, int* expert_for_source_row,
                  cudaStream_t stream) {
    CHECK_WITH_INFO(false, "FP32 x int8 MoE not supported.");
  }
  ...
  void run_moe_fc(const float* input_activations, const float* gating_output, const uint8_t* fc1_expert_weights,
                  ...
                  const uint8_t* fc2_expert_weights, const float* fc2_scales, const int num_rows, const int hidden_size,
                  const int inter_size, const int num_experts, const int k, char* workspace_ptr, float* fc2_result,
                  const bool* finished, const int active_rows, float* expert_scales,
                  int* expanded_source_row_to_expanded_dest_row, int* expert_for_source_row, cudaStream_t stream) {
    CHECK_WITH_INFO(false, "FP32 x int8 MoE not supported.");
  }
