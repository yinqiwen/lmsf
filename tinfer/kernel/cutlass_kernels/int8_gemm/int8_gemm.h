...
#include "tinfer/kernel/cutlass_extensions/epilogue/epilogue_quant_helper.h"
...
class CutlassInt8GemmRunner {
 public:
  ...
  void gemm(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col, const float* alpha_row,
 ...
 private:
  void dispatch_to_arch(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
  ...
  void run_gemm(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col, const float* alpha_row,
                T* C, int m, int n, int k, char* workspace_ptr, const size_t workspace_bytes, cudaStream_t stream);
