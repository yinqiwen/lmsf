...
#include "tinfer/kernel/cutlass_extensions/compute_occupancy.h"
#include "tinfer/kernel/cutlass_extensions/epilogue/threadblock/epilogue_tensor_op_int32.h"
#include "tinfer/kernel/cutlass_extensions/epilogue_helpers.h"
#include "tinfer/kernel/cutlass_extensions/ft_gemm_configs.h"

#include "tinfer/kernel/cutlass_extensions/gemm/kernel/default_fpA_intB_traits.h"
#include "tinfer/kernel/cutlass_extensions/gemm/kernel/gemm_with_epilogue_visitor.h"
#include "tinfer/kernel/cutlass_extensions/gemm/threadblock/default_mma.h"

#pragma GCC diagnostic pop

#include "tinfer/kernel/cutlass_kernels/cutlass_heuristic.h"
#include "tinfer/kernel/cutlass_kernels/int8_gemm/int8_gemm.h"
...
void generic_int8_gemm_kernelLauncher(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
                                      const float* alpha_row, T* C, int m, int n, int k, CutlassGemmConfig gemm_config,
...
//   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
  ...
  using AlphaColTileIterator = cutlass::epilogue::threadblock::PredicatedTileIterator<
      cutlass::epilogue::threadblock::OutputTileOptimalThreadMap<
          ...
          GemmKernel_::Epilogue::OutputTileIterator::kElementsPerAccess, cutlass::sizeof_bits<ElementCompute>::value>,
      ElementCompute>;
  ...
  if (gemm.get_workspace_size(args) > workspace_bytes) {
    FT_LOG_WARNING("Requested split-k but workspace size insufficient. Falling back to non-split-k implementation.");
  ...
  if (run_status != cutlass::Status::kSuccess) {
    std::string err_msg = "Failed to run cutlass int8 gemm. Error: " + std::string(cutlassGetStatusString(run_status));
    throw std::runtime_error("[FT Error][int8gemm Runner] " + err_msg);
  }
#else
  throw std::runtime_error(
      "[FT Error][int8gemm] FasterTransformer was built was mixed gemm support off. Please rebuild with cmake option "
...
struct dispatch_stages {
  static void dispatch(const int8_t* A, const int8_t* B, const float* alpha_col, const float* alpha_row, T* C, int m,
                       int n, int k, CutlassGemmConfig gemm_config, char* workspace, size_t workspace_bytes,
...
struct dispatch_stages<T, arch, ThreadblockShape, WarpShape, 2> {
  static void dispatch(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
                       const float* alpha_row, T* C, int m, int n, int k, CutlassGemmConfig gemm_config,
                       char* workspace, size_t workspace_bytes, cudaStream_t stream, int* occupancy = nullptr) {
    // FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    generic_int8_gemm_kernelLauncher<T, arch, ThreadblockShape, WarpShape, 2>(
...
struct dispatch_stages<T, cutlass::arch::Sm80, ThreadblockShape, WarpShape, Stages,
                       typename std::enable_if<(Stages > 2)>::type> {
  static void dispatch(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
                       const float* alpha_row, T* C, int m, int n, int k, CutlassGemmConfig gemm_config,
                       char* workspace, size_t workspace_bytes, cudaStream_t stream, int* occupancy = nullptr) {
    // FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    generic_int8_gemm_kernelLauncher<T, cutlass::arch::Sm80, ThreadblockShape, WarpShape, Stages>(
        A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, gemm_config, workspace, workspace_bytes, stream, occupancy);
  }
};

template <typename T, typename arch, typename ThreadblockShape, typename WarpShape>
void dispatch_gemm_config(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
                          const float* alpha_row, T* C, int m, int n, int k, CutlassGemmConfig gemm_config,
                          char* workspace, size_t workspace_bytes, cudaStream_t stream, int* occupancy = nullptr) {
  //   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
  switch (gemm_config.stages) {
    case 2:
      ...
      DispatcherStages2::dispatch(A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, gemm_config, workspace,
                                  workspace_bytes, stream, occupancy);
      break;
    case 3:
      using DispatcherStages3 = dispatch_stages<T, arch, ThreadblockShape, WarpShape, 3>;
      DispatcherStages3::dispatch(A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, gemm_config, workspace,
                                  workspace_bytes, stream, occupancy);
      break;
    case 4:
      using DispatcherStages4 = dispatch_stages<T, arch, ThreadblockShape, WarpShape, 4>;
      DispatcherStages4::dispatch(A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, gemm_config, workspace,
                                  workspace_bytes, stream, occupancy);
      break;
    default:
...
void dispatch_gemm_to_cutlass(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
                              const float* alpha_row, T* C, int m, int n, int k, char* workspace,
                              size_t workspace_bytes, CutlassGemmConfig gemm_config, cudaStream_t stream,
                              int* occupancy = nullptr) {
  //   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
CutlassInt8GemmRunner<T>::CutlassInt8GemmRunner() {
  ...
  sm_ = tinfer::getSMVersion();
  CUDA_CHECK(cudaDeviceGetAttribute(&multi_processor_count_, cudaDevAttrMultiProcessorCount, device));
}
...
template <typename T>
void CutlassInt8GemmRunner<T>::dispatch_to_arch(const int8_t* A, const int8_t* B, QuantMode quant_mode,
                                                const float* alpha_col, const float* alpha_row, T* C, int m, int n,
                                                int k, CutlassGemmConfig gemm_config, char* workspace_ptr,
                                                const size_t workspace_bytes, cudaStream_t stream, int* occupancy) {
  //   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
  ...
  /* } */
  if (sm_ >= 80 && sm_ < 90) {
    dispatch_gemm_to_cutlass<T, cutlass::arch::Sm80>(A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, workspace_ptr,
                                                     workspace_bytes, gemm_config, stream, occupancy);
  } else {
    throw std::runtime_error("[FT Error][CutlassInt8GemmRunner][GEMM Dispatch] Arch unsupported for CUTLASS int8 GEMM");
  }
}

template <typename T>
void CutlassInt8GemmRunner<T>::run_gemm(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_col,
                                        const float* alpha_row, T* C, int m, int n, int k, char* workspace_ptr,
  ...
  for (size_t ii = 0; ii < candidate_configs.size(); ++ii) {
    dispatch_to_arch(A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, candidate_configs[ii], workspace_ptr,
  ...
  CutlassGemmConfig chosen_config =
      estimate_best_config_from_occupancies(candidate_configs, occupancies, m, n, k, num_experts, split_k_limit,
                                            workspace_bytes, multi_processor_count_, is_weight_only);

  dispatch_to_arch(A, B, quant_mode, alpha_col, alpha_row, C, m, n, k, chosen_config, workspace_ptr, workspace_bytes,
                   stream);
}

template <typename T>
void CutlassInt8GemmRunner<T>::gemm(const int8_t* A, const int8_t* B, QuantMode quant_mode, const float* alpha_row,
                                    const float* alpha_col, T* C, int m, int n, int k, char* workspace_ptr,
                                    const size_t workspace_bytes, cudaStream_t stream) {
  //   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
