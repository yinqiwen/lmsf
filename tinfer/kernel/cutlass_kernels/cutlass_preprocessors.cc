...
#include "tinfer/kernel/cutlass_kernels/cutlass_preprocessors.h"
// #include "src/fastertransformer/utils/cuda_bf16_wrapper.h"

#include "tinfer/kernel/cutlass_extensions/gemm/kernel/mixed_gemm_B_layout.h"
...
void permute_B_rows_for_mixed_gemm(int8_t* permuted_quantized_tensor, const int8_t* quantized_tensor,
  ...
  CHECK_WITH_INFO(arch_version >= 75,
...
void subbyte_transpose_impl(int8_t* transposed_quantized_tensor, const int8_t* quantized_tensor,
  ...
  CHECK_WITH_INFO(!(col_bytes_trans % VECTOR_WIDTH) && !(col_bytes % VECTOR_WIDTH),
                  fmtstr("Number of bytes for rows and cols must be a multiple of %d. However, num_rows_bytes = %ld "
                         "and num_col_bytes = %d.",
...
void subbyte_transpose(int8_t* transposed_quantized_tensor, const int8_t* quantized_tensor,
  ...
  if (quant_type == QuantType::INT8_WEIGHT_ONLY) {
    ...
    subbyte_transpose_impl<QuantType::PACKED_INT4_WEIGHT_ONLY>(transposed_quantized_tensor, quantized_tensor, shape);
  } else {
...
void add_bias_and_interleave_int4s_inplace(int8_t* packed_int4_tensor, const size_t num_elts) {
  ...
  for (size_t ii = 0; ii < num_bytes; ++ii) {
    ...
    CHECK_WITH_INFO(transformed_first_elt >= 0 && transformed_first_elt <= 15,
...
void interleave_column_major_tensor(int8_t* interleaved_quantized_tensor, const int8_t* quantized_tensor,
  ...
  CHECK_WITH_INFO(!(num_cols % rows_per_tile),
...
void symmetric_quantize(int8_t* processed_quantized_weight, int8_t* unprocessed_quantized_weight,
                        ComputeType* scale_ptr, const WeightType* input_weight_ptr, const std::vector<size_t>& shape,
...
template void symmetric_quantize<half, float>(int8_t*, int8_t*, half*, const float*, const std::vector<size_t>&,
                                              QuantType);

template void symmetric_quantize<half, half>(int8_t*, int8_t*, half*, const half*, const std::vector<size_t>&,
                                             QuantType);

#ifdef ENABLE_BF16
template void symmetric_quantize<__nv_bfloat16, __nv_bfloat16>(int8_t*, int8_t*, __nv_bfloat16*, const __nv_bfloat16*,
                                                               const std::vector<size_t>&, QuantType);

template void symmetric_quantize<__nv_bfloat16, float>(int8_t*, int8_t*, __nv_bfloat16*, const float*,
                                                       const std::vector<size_t>&, QuantType);
#endif
...
#ifdef ENABLE_BF16
template void symmetric_quantize<__nv_bfloat16, __nv_bfloat16>(int8_t*, __nv_bfloat16*, const __nv_bfloat16*,
                                                               const std::vector<size_t>&, QuantType);

template void symmetric_quantize<__nv_bfloat16, half>(int8_t*, __nv_bfloat16*, const half*, const std::vector<size_t>&,
                                                      QuantType);

template void symmetric_quantize<half, __nv_bfloat16>(int8_t*, half*, const __nv_bfloat16*, const std::vector<size_t>&,
                                                      QuantType);

template void symmetric_quantize<__nv_bfloat16, float>(int8_t*, __nv_bfloat16*, const float*,
                                                       const std::vector<size_t>&, QuantType);
#endif
