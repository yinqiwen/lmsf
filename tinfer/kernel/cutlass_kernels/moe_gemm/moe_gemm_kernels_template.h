...
#include "tinfer/kernel/cutlass_extensions/gemm/kernel/moe_cutlass_kernel.h"
...
#include "tinfer/kernel/cutlass_kernels/moe_gemm/moe_gemm_kernels.h"
...
//   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
  ...
  if (occupancy == 0) {
    throw std::runtime_error("[FT Error][MoE Runner] GPU lacks the shared memory resources to run GroupedGEMM kernel");
  }
...
void dispatch_moe_gemm_to_cutlass(const T* A, const WeightType* B, const T* weight_scales, const T* biases, T* C,
  ...
  switch (gemm_config.tile_config) {
    ...
    case CutlassTileConfig::ChooseWithHeuristic:
      ...
      break;
    default:
      throw std::runtime_error("[FT Error][dispatch_moe_gemm_to_cutlass][SIMT] Unsupported config for float MoE gemm.");
      break;
  }
}
...
void MoeGemmRunner<T, WeightType>::moe_gemm_bias_act(const T* A, const WeightType* B, const T* weight_scales,
                                                     const T* biases, T* C, int64_t* total_rows_before_expert,
                                                     int64_t total_rows, int64_t gemm_n, int64_t gemm_k,
                                                     int num_experts, tinfer::ActivationType activation_type,
                                                     cudaStream_t stream) {
  //   FT_LOG_DEBUG(__PRETTY_FUNCTION__);
