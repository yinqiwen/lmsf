...
size_t CubKeyValueSorter::getWorkspaceSize(const size_t num_key_value_pairs) {
  ...
  cub::DeviceRadixSort::SortPairs(NULL, required_storage, null_int, null_int, null_int, null_int, num_key_value_pairs,
                                  0, num_bits_);
...
void CutlassMoeFCRunner<T, WeightType, Enable>::run_moe_fc(
    ...
    const T* fc1_expert_biases, tinfer::ActivationType fc1_activation_type, const WeightType* fc2_expert_weights,
  ...
  if (scales_required) {
    if (fc1_scales == nullptr) {
      throw std::runtime_error("[FT Error][Run MoE FC] Scales expected but scale for first matmul is a null pointer");
    } else if (fc2_scales == nullptr) {
      throw std::runtime_error("[FT Error][Run MoE FC] Scales expected but scale for second matmul is a null pointer");
    }
  } else {
    if (fc1_scales != nullptr) {
...
#ifdef ENABLE_BF16
template void topk_gating_softmax_kernelLauncher(const __nv_bfloat16*, const bool*, __nv_bfloat16*, __nv_bfloat16*,
                                                 int*, int*, const int, const int, const int, cudaStream_t);
#endif
...
template void initialize_moe_routing_kernelLauncher(const half*, half*, const int*, int*, const int, const int,
                                                    const int, const int, cudaStream_t);
#ifdef ENABLE_BF16
template void initialize_moe_routing_kernelLauncher(const __nv_bfloat16*, __nv_bfloat16*, const int*, int*, const int,
...
template void finalize_moe_routing_kernelLauncher(const __nv_bfloat16*, __nv_bfloat16*, const __nv_bfloat16*,
                                                  const __nv_bfloat16*, const __nv_bfloat16*, const int*, const int*,
                                                  const int, const int, const int, cudaStream_t);
template void finalize_moe_routing_kernelLauncher(const __nv_bfloat16*, __nv_bfloat16*, const __nv_bfloat16*,
