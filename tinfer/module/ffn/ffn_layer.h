...
#include "tinfer/kernel/activation_kernels.h"
...
#include "tinfer/kernel/matrix_vector_multiplication.h"
#include "tinfer/kernel/moe_kernels.h"
#include "tinfer/module/common/base_layer.h"
#include "tinfer/module/ffn/ffn_weight.h"
#include "tinfer/tensor/tensor.h"
#include "tinfer/utils/activation_types.h"
...
class FfnLayer : public BaseLayer {
  ...
  */
  ...
  virtual tinfer::ActivationType getActivationType() const { return tinfer::ActivationType::InvalidType; };

  void genericActivation(int m, const T* bias1, const T* bias2, const int* ia3_tasks, const T* ia3_weights,
                         const float* activation_in, const float* activation_out, const int* padding_offset,
                         const int seq_len);

 public:
  FfnLayer(size_t max_batch_size, size_t max_seq_len,
           ...
           tinfer::CudaAllocator* allocator, bool is_free_buffer_after_forward, bool sparse = false, int int8_mode = 0,
  ...
  void resetInterSize(size_t runtime_inter_size) { inter_size_ = runtime_inter_size; }

  //   virtual void forward(std::vector<fastertransformer::Tensor>* output_tensors,
  //                        const std::vector<fastertransformer::Tensor>* input_tensors, const FfnWeight<T>*
  //                        ffn_weights);
  virtual void forward(tinfer::TensorMap* output_tensors, tinfer::TensorMap* input_tensors,
...
class GeluFfnLayer : public FfnLayer<T> {
 public:
  GeluFfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head, size_t expert_num,
               size_t inter_size, cudaStream_t stream, cublasMMWrapper* cublas_wrapper,
               tinfer::CudaAllocator* allocator, bool is_free_buffer_after_forward, bool sparse = false,
               int int8_mode = 0, bool use_gated_activation = false);
 ...
 protected:
  ...
  virtual tinfer::ActivationType getActivationType() const override { return tinfer::ActivationType::Gelu; };
...
class ReluFfnLayer : public FfnLayer<T> {
 public:
  ReluFfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head, size_t expert_num,
               size_t inter_size, cudaStream_t stream, cublasMMWrapper* cublas_wrapper,
               tinfer::CudaAllocator* allocator, bool is_free_buffer_after_forward, bool sparse = false,
               int int8_mode = 0, bool use_gated_activation = false);
 ...
 protected:
  using FfnLayer<T>::stream_;
  virtual tinfer::ActivationType getActivationType() const override { return tinfer::ActivationType::Relu; };
...
class SiluFfnLayer : public FfnLayer<T> {
 public:
  SiluFfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head, size_t expert_num,
               size_t inter_size, cudaStream_t stream, cublasMMWrapper* cublas_wrapper,
               tinfer::CudaAllocator* allocator, bool is_free_buffer_after_forward, bool sparse = false,
               bool use_gated_activation = false);
 ...
 protected:
  using FfnLayer<T>::stream_;
  virtual tinfer::ActivationType getActivationType() const override { return tinfer::ActivationType::Silu; };
