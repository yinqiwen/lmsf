...
#include "tinfer/module/ffn/ffn_layer.h"

#include "tinfer/kernel/transpose_int8_kernels.h"
...
template <typename T>
void FfnLayer<T>::forward(tinfer::TensorMap* output_tensors, tinfer::TensorMap* input_tensors,
                          const FfnWeight<T>* ffn_weights) {
  // input tensors:
  ...
  CHECK(input_tensors->Size() >= 1 && input_tensors->Size() <= 5);
  CHECK(output_tensors->Size() >= 1 || output_tensors->Size() <= 4);
  bool use_moe = false;
  size_t moe_k = 0;
  ...
  CHECK((use_moe && output_tensors->IsExist("expert_scales") &&
  ...
  CHECK(!(use_gated_activation && use_moe));
  ...
  if (use_moe) {
    // PUSH_RANGE("FFN moe");
    CHECK(ia3_tasks == nullptr);
    cublas_wrapper_->Gemm(CUBLAS_OP_N, CUBLAS_OP_N, expert_num_, m, hidden_units_, ffn_weights->gating_weight.kernel,
                          expert_num_, input_tensor, hidden_units_, moe_gates_buf_, expert_num_);

    if (int8_mode_ == 0) {
      moe_fc_runner_->run_moe_fc(input_tensor, moe_gates_buf_, ffn_weights->intermediate_weight.kernel,
                                 ffn_weights->intermediate_weight.weight_only_quant_scale,
                                 ffn_weights->intermediate_weight.bias, activation_type,
                                 ffn_weights->output_weight.kernel, ffn_weights->output_weight.weight_only_quant_scale,
                                 m, hidden_units_, inter_size_, expert_num_, moe_k, moe_fc_workspace_, output_tensor,
                                 expert_scales, permuted_rows, permuted_experts, stream_);
    } else if (int8_mode_ == 1) {
      CHECK_WITH_INFO(moe_int8_weight_only_fc_runner_.get() != NULL, "weight only runner was not initialized.");

      CHECK(ffn_weights->intermediate_weight.int8_kernel != NULL &&
            ffn_weights->intermediate_weight.weight_only_quant_scale != NULL);

      CHECK(ffn_weights->output_weight.int8_kernel != NULL &&
            ffn_weights->output_weight.weight_only_quant_scale != NULL);

      moe_int8_weight_only_fc_runner_->run_moe_fc(
          input_tensor, moe_gates_buf_, reinterpret_cast<const uint8_t*>(ffn_weights->intermediate_weight.int8_kernel),
          ffn_weights->intermediate_weight.weight_only_quant_scale, ffn_weights->intermediate_weight.bias,
          activation_type, reinterpret_cast<const uint8_t*>(ffn_weights->output_weight.int8_kernel),
          ffn_weights->output_weight.weight_only_quant_scale, m, hidden_units_, inter_size_, expert_num_, moe_k,
          moe_fc_workspace_, output_tensor, expert_scales, permuted_rows, permuted_experts, stream_);
    } else {
      CHECK_WITH_INFO(false, "Invalid int8 mode for MoE");
    }
  ...
  if (use_sparse_gemm) {
    CHECK(!use_gated_activation);
#ifdef SPARSITY_ENABLED
    cublas_wrapper_->SpGemm(CUBLAS_OP_N, CUBLAS_OP_N, inter_size_, m_padded, hidden_units_,
                            ffn_weights->intermediate_weight.sp_kernel, input_tensor, inter_buf_);
#endif
  } else {
    if (int8_mode_ == 1) {
      CHECK_WITH_INFO(weight_only_int8_fc_runner_.get() != NULL, "weight only runner was not initialized.");
      CHECK(ffn_weights->intermediate_weight.int8_kernel != NULL &&
            ffn_weights->intermediate_weight.weight_only_quant_scale != NULL);
      ...
      if (ia3_tasks == nullptr && !use_gated_activation) {
        ...
        weight_only_int8_fc_runner_->gemm_bias_act(
            input_tensor, reinterpret_cast<const uint8_t*>(ffn_weights->intermediate_weight.int8_kernel),
            ffn_weights->intermediate_weight.weight_only_quant_scale, ffn_weights->intermediate_weight.bias, inter_buf_,
            m, inter_size_, hidden_units_, activation_type, mixed_gemm_workspace_, mixed_gemm_ws_bytes_, stream_);
      } else {
        // Otherwise, let FT handle activation
        weight_only_int8_fc_runner_->gemm(
            input_tensor, reinterpret_cast<const uint8_t*>(ffn_weights->intermediate_weight.int8_kernel),
            ffn_weights->intermediate_weight.weight_only_quant_scale, inter_buf_, m, inter_size_, hidden_units_,
            mixed_gemm_workspace_, mixed_gemm_ws_bytes_, stream_);

        if (use_gated_activation) {
          CHECK(ffn_weights->intermediate_weight2.int8_kernel != NULL &&
                ffn_weights->intermediate_weight2.weight_only_quant_scale != NULL);

          weight_only_int8_fc_runner_->gemm(
              input_tensor, reinterpret_cast<const uint8_t*>(ffn_weights->intermediate_weight2.int8_kernel),
              ffn_weights->intermediate_weight2.weight_only_quant_scale, inter_buf_2_, m, inter_size_, hidden_units_,
              mixed_gemm_workspace_, mixed_gemm_ws_bytes_, stream_);
        }
      }
    } else if (int8_mode_ == 2) {
      CHECK(!use_gated_activation);
      cublas_wrapper_->Int8Gemm(inter_size_, m, hidden_units_, ffn_weights->intermediate_weight.int8_kernel,
                                hidden_units_, input_tensors->GetPtr<int8_t>("ffn_input"), hidden_units_,
                                reinterpret_cast<int8_t*>(inter_buf_), inter_size_,
                                ffn_weights->intermediate_weight.scale_inter);
    } else {
      cublas_wrapper_->Gemm(CUBLAS_OP_N, CUBLAS_OP_N, inter_size_, m, hidden_units_,
                            ffn_weights->intermediate_weight.kernel, inter_size_, input_tensor, hidden_units_,
                            inter_buf_, inter_size_);
      if (use_gated_activation) {
        cublas_wrapper_->Gemm(CUBLAS_OP_N, CUBLAS_OP_N, inter_size_, m, hidden_units_,
                              ffn_weights->intermediate_weight2.kernel, inter_size_, input_tensor, hidden_units_,
                              inter_buf_2_, inter_size_);
      }
    }
  }
  ...
  if (int8_mode_ != 1 || ia3_tasks != nullptr || use_gated_activation) {
    ...
    genericActivation(
        m, ffn_weights->intermediate_weight.bias,
        use_gated_activation ? ffn_weights->intermediate_weight2.bias : nullptr,
        input_tensors->At("ia3_tasks", {tinfer::MEMORY_GPU, tinfer::TYPE_INT32, {}, nullptr}).GetPtr<const int>(),
        ffn_weights->ia3_weight.kernel, int8_mode_ == 2 ? ffn_weights->intermediate_weight.scale_out : (float*)nullptr,
        int8_mode_ == 2 ? ffn_weights->output_weight.scale : (float*)nullptr,
        input_tensors->GetPtr<int>("padding_offset", nullptr), input_tensors->GetVal<int>("seq_len", 1));
    // POP_RANGE;
  }
  ...
  if (use_sparse_gemm) {
#ifdef SPARSITY_ENABLED
    cublas_wrapper_->SpGemm(CUBLAS_OP_N, CUBLAS_OP_N, hidden_units_, m_padded, inter_size_,
                            ffn_weights->output_weight.sp_kernel, inter_buf_, output_tensor);
#endif
  } else {
    if (int8_mode_ == 1) {
      CHECK_WITH_INFO(weight_only_int8_fc_runner_.get() != NULL, "weight only runner was not initialized.");
      CHECK(ffn_weights->output_weight.int8_kernel != NULL &&
            ffn_weights->output_weight.weight_only_quant_scale != NULL);
      weight_only_int8_fc_runner_->gemm(
          inter_buf_, reinterpret_cast<const uint8_t*>(ffn_weights->output_weight.int8_kernel),
          ffn_weights->output_weight.weight_only_quant_scale, output_tensor, m, hidden_units_, inter_size_,
          mixed_gemm_workspace_, mixed_gemm_ws_bytes_, stream_);
    } else if (int8_mode_ == 2) {
      int8_fc_runner_->gemm(reinterpret_cast<int8_t*>(inter_buf_), ffn_weights->output_weight.int8_kernel,
                            QuantMode::PerTensorQuant, ffn_weights->output_weight.scale_inter,
                            ffn_weights->output_weight.scale_out, output_tensors->GetPtr<T>("ffn_output"), m,
                            hidden_units_, inter_size_, nullptr, 0, stream_);
    } else {
      cublas_wrapper_->Gemm(CUBLAS_OP_N, CUBLAS_OP_N, hidden_units_, m, inter_size_, ffn_weights->output_weight.kernel,
                            hidden_units_, inter_buf_, inter_size_, output_tensor, hidden_units_);
    }
  }
  CUDA_SYNC_CHECK();
  // POP_RANGE;
...
template <typename T>
FfnLayer<T>::FfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head,
                      size_t expert_num, size_t inter_size, cudaStream_t stream, cublasMMWrapper* cublas_wrapper,
                      tinfer::CudaAllocator* allocator, bool is_free_buffer_after_forward, bool sparse, int int8_mode,
  ...
  if (int8_mode_ == 0) {
    ...
    CHECK_WITH_INFO(!(std::is_same<T, float>::value), "Weight only quant not supported for fp32.");
...
FfnLayer<T>::FfnLayer(FfnLayer<T> const& ffn_layer)
    : BaseLayer(ffn_layer.stream_, ffn_layer.cublas_wrapper_, ffn_layer.allocator_,
                ffn_layer.is_free_buffer_after_forward_, ffn_layer.cuda_device_prop_, ffn_layer.sparse_),
...
void FfnLayer<T>::allocateBuffer(size_t token_num, int moe_k, bool use_moe) {
  ...
  if (use_moe) {
    ...
    if (int8_mode_ == 0) {
      CHECK_WITH_INFO(moe_fc_runner_.get() != NULL, "moe runner was not initialized.");
      ws_size_moe = moe_fc_runner_->getWorkspaceSize(token_num, hidden_units_, inter_size_, expert_num_, moe_k);
    } else if (int8_mode_ == 1) {
      CHECK_WITH_INFO(moe_int8_weight_only_fc_runner_.get() != NULL, "weight only moe runner was not initialized.");
      ws_size_moe =
          moe_int8_weight_only_fc_runner_->getWorkspaceSize(token_num, hidden_units_, inter_size_, expert_num_, moe_k);
    }
...
#define INVOKE_GENERIC_ACT(ACT)                                                                                \
  invokeGenericActivation<ACT>(inter_buf_, bias1, inter_buf_2_, bias2, ia3_tasks, ia3_weights, m, inter_size_, \
                               int8_mode_, activation_in, activation_out, padding_offset, seq_len, stream_);

template <typename T>
void FfnLayer<T>::genericActivation(int m, const T* bias1, const T* bias2, const int* ia3_tasks, const T* ia3_weights,
                                    const float* activation_in, const float* activation_out, const int* padding_offset,
                                    const int seq_len) {
  ...
  switch (getActivationType()) {
    case tinfer::ActivationType::Gelu:
    case tinfer::ActivationType::GeGLU:
      if (inter_buf_2_ == nullptr && int8_mode_ <= 1) {
        invokeAddBiasGeluV2(inter_buf_, bias1, ia3_tasks, ia3_weights, padding_offset, seq_len, m, inter_size_,
                            stream_);
      } else {
        INVOKE_GENERIC_ACT(GeluActivation);
      }
      break;
    case tinfer::ActivationType::Relu:
    case tinfer::ActivationType::ReGLU:
      INVOKE_GENERIC_ACT(ReluActivation);
      break;
    case tinfer::ActivationType::Silu:
    case tinfer::ActivationType::SiGLU:
      INVOKE_GENERIC_ACT(SiluActivation);
      break;
    case tinfer::ActivationType::Identity:
...
template <typename T>
GeluFfnLayer<T>::GeluFfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head,
                              size_t expert_num, size_t inter_size, cudaStream_t stream,
                              cublasMMWrapper* cublas_wrapper, tinfer::CudaAllocator* allocator,
                              bool is_free_buffer_after_forward, bool sparse, int int8_mode, bool use_gated_activation)
    : FfnLayer<T>(max_batch_size, max_seq_len, head_num, size_per_head, expert_num, inter_size, stream, cublas_wrapper,
                  allocator, is_free_buffer_after_forward, sparse, int8_mode, use_gated_activation) {}
...
template <typename T>
ReluFfnLayer<T>::ReluFfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head,
                              size_t expert_num, size_t inter_size, cudaStream_t stream,
                              cublasMMWrapper* cublas_wrapper, tinfer::CudaAllocator* allocator,
                              bool is_free_buffer_after_forward, bool sparse, int int8_mode, bool use_gated_activation)
    : FfnLayer<T>(max_batch_size, max_seq_len, head_num, size_per_head, expert_num, inter_size, stream, cublas_wrapper,
...
template <typename T>
SiluFfnLayer<T>::SiluFfnLayer(size_t max_batch_size, size_t max_seq_len, size_t head_num, size_t size_per_head,
                              size_t expert_num, size_t inter_size, cudaStream_t stream,
                              cublasMMWrapper* cublas_wrapper, tinfer::CudaAllocator* allocator,
                              bool is_free_buffer_after_forward, bool sparse, bool use_gated_activation)
    : FfnLayer<T>(max_batch_size, max_seq_len, head_num, size_per_head, expert_num, inter_size, stream, cublas_wrapper,
                  allocator, is_free_buffer_after_forward, sparse, 0, use_gated_activation) {}
