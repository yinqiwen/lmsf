...
#if ENABLE_BF16
template <> inline __device__ __nv_bfloat162 ldg(const __nv_bfloat162 *val) {
...
template <> inline __device__ __nv_bfloat16 ldg(const __nv_bfloat16 *val) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 800
  return val[0];
#else
  return __ldg(val);
...
template <typename T> inline __device__ T add(T a, T b, T c) {
  return a + b + c;
}
...
template <typename T> inline __device__ T mul(T a, T b, T c) {
  return a * b * c;
}
...
template <typename T> inline __device__ T fma(T a, T b, T c, T d) {
  return a * b * c + d;
}
...
template <typename T> inline __device__ T fma(T a, T b, T c) {
  return a * b + c;
}
...
#if ENABLE_BF16
template <> inline __device__ __nv_bfloat162 hexp2(__nv_bfloat162 a) {
  return bf16exp2(a);
}
#endif // ENABLE_BF16
...
cuda_cast<int16_t, __nv_bfloat162>(__nv_bfloat162 val) {
...
cuda_cast<__nv_bfloat162, __nv_bfloat16>(__nv_bfloat16 val) {
...
template <>
__device__ inline __nv_bfloat162
cuda_cast<__nv_bfloat162, int16_t>(int16_t val) {
  union {
    int8_t int8[2];
    int16_t int16;
  };
...
cuda_cast<float2, __nv_fp8x2_e4m3>(__nv_fp8x2_e4m3 val) {
  return bf1622float2(fp8x2_e4m3_to_bfloat2(&val));
}
template <>
__device__ inline __nv_fp8x2_e4m3
cuda_cast<__nv_fp8x2_e4m3, float2>(float2 val) {
...
cuda_cast<__nv_fp8_e4m3, __nv_bfloat16>(__nv_bfloat16 val) {
  return __nv_fp8_e4m3(val);
}
template <>
...
template <>
__device__ inline __nv_bfloat162
cuda_cast<__nv_bfloat162, __nv_fp8x2_e4m3>(__nv_fp8x2_e4m3 val) {
