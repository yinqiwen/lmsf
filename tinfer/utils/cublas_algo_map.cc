...
#endif
                &exec_time) != EOF) {
    if (dataType != tinfer::FLOAT_DATATYPE && dataType != tinfer::HALF_DATATYPE &&
        dataType != tinfer::BFLOAT16_DATATYPE && dataType != tinfer::INT8_DATATYPE &&
        dataType != tinfer::FP8_DATATYPE) {
      printf("[WARNING][readAlgoFromConfig] wrong dataType %d!\n", dataType);
      continue;
    }
    cublasAlgoConfig_t markStr{batchCount2, m2, n2, k2, static_cast<tinfer::CublasDataType>(dataType)};
...
bool cublasAlgoMap::isExist(const int batch_count, const int m, const int n, const int k,
                            const tinfer::CublasDataType data_type) {
  cublasAlgoConfig_t mark{batch_count, n, m, k, data_type};
  return algo_map_.find(mark) != algo_map_.end();
}

cublasLtMatmulAlgo_info cublasAlgoMap::getAlgo(const int batch_count, const int m, const int n, const int k,
                                               const tinfer::CublasDataType data_type) {
  cublasAlgoConfig_t mark{batch_count, n, m, k, data_type};
  if (algo_map_.find(mark) != algo_map_.end()) {
    ...
    tmp_algo.algoId =
        static_cast<int>(data_type == tinfer::FLOAT_DATATYPE ? CUBLAS_GEMM_DEFAULT : CUBLAS_GEMM_DEFAULT_TENSOR_OP);
...
void cublasAlgoMap::loadSpGemmConfig() {
  ...
  while (fscanf(fd, "%d %d %d %d %d ### %d %d %d %d %d %f\n", &batch_size, &seq_len, &head_num, &size_per_head,
                &data_type, &batchCount, &m, &n, &k, &algoId, &exec_time) != EOF) {
    char mark[256];
